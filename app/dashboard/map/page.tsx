...
export default async function MapPage() {
  ...
  return (
    <>
      ...
      <Panel title="Persistent place map" description="静态图由服务端代理；AMAP key 不会出现在浏览器 URL。">
        <div className="relative aspect-[16/7] min-h-72 overflow-hidden rounded-md border border-white/[0.07] bg-[#080a0d]">
          {process.env.AMAP_WEB_API_KEY ? <Image src="/api/admin/world/map-image" alt="Mira 在北京的已知地点静态地图" fill unoptimized className="object-cover opacity-80" /> : <div className="grid h-full place-items-center p-6 text-center"><div><MapPinned className="mx-auto size-8 text-cyan-300/50" /><p className="mt-3 text-sm text-zinc-400">尚未配置 AMAP_WEB_API_KEY</p><p className="mt-1 text-xs text-zinc-600">地点列表仍可用；配置后这里显示服务端静态图。</p></div></div>}
        </div>
      </Panel>
