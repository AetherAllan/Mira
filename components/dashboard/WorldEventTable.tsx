    ...
    <div className="space-y-3">
      {events.map((event) => (
        <article key={event.id} className="rounded-md border border-white/[0.07] bg-black/15 p-4">
          <div className="flex items-start justify-between gap-4"><div><p className="text-sm text-zinc-200">{event.title}</p><p className="mt-1 font-mono text-[9px] text-zinc-700">seed {event.seedId ?? "generated"}</p></div><time className="font-mono text-[9px] text-zinc-600">{formatDate(event.createdAt)}</time></div>
